...
import { checkUserAddressesExistence } from "@/backend/database/drizzle/functions-and-queries/users/user-db-functions-and-queires";
...
export async function createUserAddress(userAddress: UserAddressType) {
  ...
  const currentNUmberOfUSerAddresses =
    await checkUserAddressesExistence(userId);
