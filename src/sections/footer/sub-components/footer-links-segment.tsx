  ...
  return (
    <nav key={linkList.label}>
      <div className="flex flex-row md:flex-col w-26 md:w-64">
        <div
          className={`${LIGHT_THEME_FOOTER_LABELS} ${DARK_THEME_FOOTER_LABELS} me-2 md:mb-2`}
          id="footerHeading">
          {linkList.label}
        </div>
        <ul className="list-none flex flex-row md:flex-col  md:space-y-2" aria-label="List of Links">
          {linkList.links?.map((link, index) => (
            <li className="flex" key={index}>
              <Link
                className={`flex ${LIGHT_THEME_FOOTER_LINK} ${DARK_THEME_FOOTER_LINK} pb-[1px] border-b-[1px] border-solid border-transparent transition-all duration-300 hover:border-current`}
